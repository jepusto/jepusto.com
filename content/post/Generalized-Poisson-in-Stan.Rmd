...
It looks like the estimated dispersion from the generalized Poisson model might be biased towards one by a little bit. To get a better sense of this, I'll run some posterior predictive checks, using the quasi-likelihood dispersion as a summary statistic:
...
Both the double Poisson and the generalized Poisson models generate data with levels of dispersion similar to the observed data.
...
Maybe we could see it if we looked at smoothed plots of the squared residuals versus $X$, but I can't find an easy way to do that with the `bayesplot` functions I've used above. I'll leave it as an exercise for you, dear reader, because I need to get to bed.
