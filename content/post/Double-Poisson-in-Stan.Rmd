...
The function below computes the un-normalized cumulative distribution function (i.e., without the scaling constant $c(\mu, \phi)$) over the range $x = 0,...,m$ as follows: 

* Compute $f(x | \mu, \phi)$ for $x = 0,1,2,...$, without the scaling constant $c(\mu, \phi)$
* Take $F(0 | \mu, \phi) = f(0 | \mu, \phi)$ and accumulating $F(x | \mu, \phi) = F(x - 1 | \mu, \phi) + f(x | \mu, \phi)$ for $x = 0,...,m$.
* Checking if $f(x | \mu, \phi) / F(x | \mu, \phi)$ is small (less than $10^{-8}$), in which case accumulation stops at the value $n$.
...
test_rng <- 
  ...
  mutate(
    ...
    gamlss_F = 10000 * pDPO(q = .x, mu = mu, sigma = 1 / phi)
  )
```
...
\end{aligned}
$$
To make things interesting, I'll set the dispersion parameter to $1 / \phi = 0.7$ so that the outcome is _under_-dispersed relative to the Poisson. 
...
summary(quasi_fit)
```
This approach recovers the data-generating parameters pretty well, with a dispersion estimate of `r round(summary(quasi_fit)$dispersion, 3)` compared to the true dispersion parameter of `r phi_inv`. The intercept is under-estimated a little bit, and the slope is over-estimated by a touch.
...
The argument `vars = "vint1[n]"` is to allow passing a value for the `maxval` argument of the `dpo_` stan functions.
...
DPO_fit <- 
  brm(
    ...
    cores = 4,
    seed = 20230913,
    save_model = "DPO-fit.stan"
  )

summary(DPO_fit)
```
The regression coefficient estimates are basically identical to those from the Poisson and negative-binomial models. However, we get a posterior for $\phi$ that corresponds to _under_-dispersion. Here's the posterior for the dispersion (i.e., $1 / \phi$):
...
To calculate LOOIC for the double-Poisson model, I first need to provide a `log_lik` function that `brms` can use:
...
To do posterior predictive checks, I need to provide a `posterior_predict` function that `brms` can use:
```{r}
...
Now I can compute posterior predictions for the double-Poisson model and make pretty pictures of them, along with corresponding plots for the Poisson and negative-binomial models.

```{r PPDs, fig.width = 8, fig.height = 6}
Yrep_Poisson <- posterior_predict(Poisson_fit, draws = 100) 
color_scheme_set("blue")
Poisson_root <- ppc_rootogram(dat$Y, Yrep_Poisson) + labs(title = "Poisson")

Yrep_negbin <- posterior_predict(negbin_fit, draws = 100)
color_scheme_set("green")
negbin_root <- ppc_rootogram(dat$Y, Yrep_negbin) + labs(title = "Negative-binomial")

Yrep_dpo <- posterior_predict(DPO_fit, draws = 100)
color_scheme_set("purple")
dpo_root <- ppc_rootogram(dat$Y, Yrep_dpo) + labs(title = "Double-Poisson")

dpo_root / Poisson_root / negbin_root &
  theme_minimal()
```
The differences in predicted frequencies are not at all obvious from these plots.
