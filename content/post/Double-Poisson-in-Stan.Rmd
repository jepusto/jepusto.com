...
To be clear up front, what I present is more complicated than really necessary because of these existing R functions to simulate values from the double-Poisson---we can just use the functions from `gamlss.dist` for purposes of posterior predictive checks (about which more below). 
I'm doing everything in Stan as an excuse to learn more about the language, which I haven't used much up until today.
I should also note that the [`bamlss` package](http://www.bamlss.org/index.html) provides similar functionality and can be combined with `gamlss.dist` to accomplish basically the same thing as I'm going to do here.
...
The `brms` package implements the negative binomial using the rate parameterization and a log transformation, so the `shape` parameter corresponds to the log of the inverse dispersion. Thus, a large shape parameter (as in the above fit) imples dispersion that is very, very close to one (i.e., close to the Poisson).
...
```
The function above uses my custom `dpo_rng()` from Stan. Of course, I could have saved a bunch of trouble by just using `gamlss.dist::rDPO()` instead.
...
```
Still kind of subtle, I suppose.
