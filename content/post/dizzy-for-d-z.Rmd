...
For basic between-group designs, the only variances in the model are the within-group variances, so the choice is limited to a) the population variance, assuming it is homogeneous across groups, b) the variance of one group, or c) the average of the variances in each group. In most applications, homogeneity is assumed (often without much reflection), version (a) of Cohen's $d$ is calculated, and the meta-analyst can go along their merry way. For sake of succinctness, I'll call this effect size $d_{b}$, with $b$ indicating the basic between-group comparison.
...
$$
Thus, under my admittedly simplistic assumptions, a meta-analysis of observed score Cohen's $d_{av}$ estimates will be biased (downward) for the overall average effect in the true-score distribution. That's bad, of course. 

On top of bias, variability in $A_{av}$ will also induce additional heterogeneity in the distribution of observed score effect sizes. To measure this additional heterogeneity, I will look at the coefficient of variation in $\tilde\delta_{av}$ so that the consequences for heterogeneity are separate from the consequences for bias. Under the assumption that $A_{av}$ is independent of $\delta_{av}$, 
$$
...
Under both of these scenarios, the observed-score $\tilde\delta_z$ is substantially more sensitive to heterogeneity of measurement procedures than is $\tilde\delta_{av}$. The implication is that $\tilde\delta_{av}$ should be preferred over $\tilde\delta_z$ as a metric for meta-analyis.
